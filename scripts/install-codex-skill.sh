#!/usr/bin/env bash
# Install the mcping Codex skill into $CODEX_HOME/skills (default ~/.codex/skills).
#
# Modes:
#   copy|link     use local skills/mcping from this checkout (dev)
#   release       download skill from GitHub (CntierTeam/mcping)
#
# Usage:
...
SRC="$REPO_ROOT/skills/mcping"
CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"
DEST_ROOT="${CODEX_HOME}/skills"
DEST="$DEST_ROOT/mcping"
MODE="${1:-copy}"
REPO="${REPO:-CntierTeam/mcping}"
...
case "$MODE" in
  ...
  release)
    ...
    skill_src="$root/skills/mcping"
    [[ -f "$skill_src/SKILL.md" ]] || { echo "error: skills/mcping not in archive" >&2; exit 1; }
